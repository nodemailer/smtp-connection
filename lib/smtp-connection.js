...
SMTPConnection.prototype.login = function(authData, callback) {
    ...
    return callback(new Error('Unknown authentication method - ' + authMethod));
};
...
SMTPConnection.prototype._processResponse = function() {
    ...
    if (typeof action === 'function') {
        ...
        return this._onError(new Error('Unexpected Response'));
    }
};
...
SMTPConnection.prototype._actionAUTH_LOGIN_USER = function(str, callback) {
    if (str !== '334 VXNlcm5hbWU6') {
        callback(new Error('Invalid login sequence while waiting for "334 VXNlcm5hbWU6":\n' + str));
        return;
    }
...
SMTPConnection.prototype._actionAUTH_CRAM_MD5 = function(str, callback) {
    ...
    if (!challengeMatch) {
        callback(new Error('Invalid login sequence while waiting for server challenge string:\n' + str));
        return;
    } else {
...
SMTPConnection.prototype._actionAUTH_CRAM_MD5_PASS = function(str, callback) {
    if (!str.match(/^235\s+/)) {
        callback(new Error('Invalid login sequence while waiting for "235":\n' + str));
        return;
    }
...
SMTPConnection.prototype._actionAUTH_LOGIN_PASS = function(str, callback) {
    if (str !== '334 UGFzc3dvcmQ6') {
        callback(new Error('Invalid login sequence while waiting for "334 UGFzc3dvcmQ6":\n' + str));
        return;
    }
...
SMTPConnection.prototype._actionMAIL = function(str, callback) {
    ...
    if (Number(str.charAt(0)) !== 2) {
        callback(new Error('Mail command failed:\n' + str));
        return;
    }

    if (!this._envelope.rcptQueue.length) {
        callback(new Error('Can\'t send mail - no recipients defined'));
    } else {
...
SMTPConnection.prototype._actionRCPT = function(str, callback) {
    ...
    if (!this._envelope.rcptQueue.length) {
        if (this._envelope.rejected.length < this._envelope.to.length) {
            ...
            callback(new Error('Can\'t send mail - all recipients were rejected'));
            return;
        }
    } else {
        this._envelope.curRecipient = this._envelope.rcptQueue.shift();
...
SMTPConnection.prototype._actionDATA = function(str, callback) {
    ...
    if ([2, 3].indexOf(Number(str.charAt(0))) < 0) {
        callback(new Error('Data command failed:\n' + str));
        return;
    }
...
SMTPConnection.prototype._actionStream = function(str, callback) {
    if (Number(str.charAt(0)) !== 2) {
        // Message failed
        callback(new Error('Message failed:\n' + str));
    } else {
