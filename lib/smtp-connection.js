...
SMTPConnection.prototype.connect = function (connectCallback) {
    if (typeof connectCallback === 'function') {
        this.once('connect', function () {
            this.logger.debug('SMTP handshake finished');
            connectCallback();
        }.bind(this));
    }
...
SMTPConnection.prototype.close = function () {
    ...
    this.logger.debug('Closing connection to the server using "%s"', closeMethod);
...
SMTPConnection.prototype._onConnect = function () {
    ...
    this.logger.info('%s established to %s:%s', this.secure ? 'Secure connection' : 'Connection', this._socket.remoteAddress, this._socket.remotePort);
...
SMTPConnection.prototype._createSendStream = function (callback) {
    ...
    if (this.options.debug) {
        ...
        dataStream.on('end', function () {
            this.logger.debug('C: <transmitted %s (source %s) bytes>', dataStream.outByteCount, dataStream.inByteCount);
        }.bind(this));
    }
...
SMTPConnection.prototype._actionSTARTTLS = function (str) {
    ...
    this._upgradeConnection((function (err, secured) {
        ...
        this.logger.info('Connection upgraded with STARTTLS');
...
SMTPConnection.prototype._actionAUTH_CRAM_MD5_PASS = function (str, callback) {
    ...
    this.logger.info('User %s authenticated', JSON.stringify(this._user));
...
SMTPConnection.prototype._actionAUTHComplete = function (str, isRetry, callback) {
    ...
    if (str.charAt(0) !== '2') {
        this.logger.info('User %s failed to authenticate', JSON.stringify(this._user));
 ...
 */
SMTPConnection.prototype._createDefaultLogger = function () {
